...
def add(cnn, table_name):
    ...
    if(os.path.isfile(c.path_ouputs_new + table_name + ".csv")):        
        ...
        # dependencies_tmp = dependencies[dependencies["parent_table"] == table_name]
        ...
        if(dependencies_tmp.shape[0] > 0):
            for d in dependencies_tmp.itertuples(index=True, name='Pandas'):
                ...
                print("parent seleccionado: \n", parent)  

                parent[[field_parent]] = parent[[field_parent]].astype(str)

                # print(parent[field_parent].to_frame())
                print("parent seleccionado en df: \n", parent[[field_parent]]) 
                #print("parent seleccionado: \n", parent)  

                parent[["id"]] = parent[["id"]].astype(str)

                print("parent seleccionado en id: \n", parent[["id"]]) 
                new_data[[field_child]] = new_data[[field_child]].astype(str)

                print("parent seleccionado en new_data[field_child]: \n", new_data[[field_child]] )
                # Mergin data
                ...
                new_data.columns = new_data.columns.str.replace('^id$',field_child, regex=True)                
...
tables = ["far_farms", "far_plots", "far_production_events"]
surveys = ["far_responses_date", "far_responses_text", "far_responses_numeric"]
...
# pd.read_sql_table('far_farms', db_connection)  
